...
SELECT * FROM TBL_USER; -- 6
SELECT * FROM TBL_POST; -- 6
SELECT * FROM TBL_REPLY; -- 9
...
FROM TBL_USER
WHERE ID IN (
	SELECT USER_ID AS "댓글 단 유저"
	FROM TBL_REPLY
	)
	AND ID IN
	(	
	SELECT USER_ID AS "게시글 단 유저"
	FROM TBL_POST
	);
...
SELECT *
FROM TBL_USER
WHERE USER_ADDRESS LIKE '경기도%' AND ID IN (
	SELECT USER_ID
	FROM TBL_REPLY
);
...
SELECT *
FROM TBL_USER
ORDER BY USER_BIRTH DESC;--오름차순 = 나이가 많은 순
...
--평균 댓글 개수보다 많이 달린 게시글 (1,4번)  -> POSTID RETURN
SELECT POST_ID
FROM TBL_REPLY
GROUP BY POST_ID
HAVING COUNT(POST_ID) > 1.8;
